...
| **B. MCP** | 代码**完全没有**（仅 docs 提到 zleap-**作为** server 对外）；`BindingTargetType`={namespace,source}；`SoulBinding` 通用但无 config 列 | 无 MCP 客户端；绑定无法指向 MCP server |
...
- **zleap 作 MCP 服务器**（补充，`docs/architecture/plugins-and-mounts.md` 已有蓝图）：把 `search_context`/`get_entity`/`remember` 用 Python `mcp` SDK 暴露成 stdio server，token-scoped 到某 soul，供 Claude Desktop/Cursor 挂载。
...
| 能力 | 旧版 SAG（TS） | 新版 zleap（Python/Next） |
|---|---|---|
...
| MCP | `@modelcontextprotocol/sdk`（server+client, stdio） | Python `mcp` SDK：client 适配远端工具为 `Tool`；server 暴露 zleap 工具 |
...
2. **MCP 绑定**：`tools/mcp.py` 客户端 + `BindingTargetType.MCP_SERVER` + `resolve_tools` + 前端绑定 UI；（可选）zleap 作 MCP server 对外。
