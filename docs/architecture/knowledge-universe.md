...
本版本只有一套生产协议和一套时间窗模型。时间线只返回 `schema_version: 3` 的原子事件结构，
每个事件包携带快照内稳定的探索序数（`ordinal`，0 = 最新端）与来源快照总量（`total_events`）；
前端偏好只读取 v5 存储键，不迁移旧结构。

## 前端模块边界

依赖方向固定为“协调器 → 领域模型 / 展示组件 → 通用库”，禁止领域模型反向依赖 React 状态：

- `knowledge-universe.tsx` 是会话协调器：持有请求生命周期、快照切换、工作集提交和场景事件接线，
  不定义确定性布局算法，也不实现可复用浮层 UI。
- `knowledge-universe-model.ts` 是无 React 状态的领域与投影模型：维护探索会话结构、时间窗同步、
  bundle 保护、稳定布局和场景签名。相同输入必须得到相同输出，可直接单元测试。
- `knowledge-universe-overlays.tsx` 只负责进度与图标控件；数字格式器按语言和记数法复用，
  行级进度组件使用稳定 props 避免协调器更新引发无关重绘。
- `universe-scene-contract.ts` 是 React 协调器、投影模型与 WebGL 场景之间唯一的数据/命令协议；
  只允许类型依赖，不加载 Three.js、DOM 或场景实现，因此领域测试不必初始化渲染器。
- `universe-scene.tsx` 是轻量 React 适配器：只负责客户端装载、国际化文本、生命周期和回调接线；
  `universe-scene-engine.ts` 才持有 WebGL/DOM 引擎，并且不访问业务 API。来源节点建立稳定索引，
  动画、LOD 和力同步热路径复用索引与 Three.js 临时向量，禁止逐帧构造来源数组、Map 或投影向量。

新增逻辑先判断归属：网络与会话事务留在协调器，确定性计算进入 model，跨层结构进入 contract，
纯展示进入 overlays，GPU/DOM 场景同步进入 scene。跨层调用必须通过显式类型和回调，不能读取
对方内部 ref。
...
- 每个信息源内部有一条**计数轴**：深度 = 事件在该源 canonical 探索序中的快照序数 ×
  固定每事件长度（60 世界单位）。canonical 序由后端定义（时间倒序，同刻按抽取器的全源
  叙事 rank）——日记按时间倒序探索，整本导入的书按阅读顺序探索；同刻塌缩不存在。
- **源内滚轮 = 沿计数轴飞行**（时间即飞行）：滚轮冲量驱动相机带惯性地沿轴前进/后退，
  轴两端是墙。相机接近可见窗口边缘时窗口 fire-and-forget 跟随翻页（快速飞行按速度提前），
  相机永不等待数据；未加载的页在其轴位就地凝现，退休的页原地淡出。拖拽 = 旋转/平移，
  Ctrl/Cmd+滚轮（pinch）= 缩放，点击 = 锁定/聚焦——一个物理手势一个语义。总览态（未入源）
  滚轮仍是缩放。
- 近大远小是**相机相对**的：呈现尺寸/透明度按「节点深度 − 飞行深度」每帧计算，相机到达
  之处必然完全在场，前方大气透视渐隐但保有可见下限，身后快速淡出。不存在按绝对年龄静态
  压暗的模式，也没有任何可切换的双模。
- 底部按钮仍可整页前进/回退（移动端与精确导航的兜底）；按钮/搜索翻页让相机滑翔到新窗口，
  飞行自己翻的页不回拉相机。
- 浏览态星云是该源的探索走廊：同一批粒子在总览是星系形态，入源后由 GPU 按细节混合拉伸为
  沿计数轴的走廊尘埃——它就是尚未加载的历史；已加载窗口带内的尘埃让位给真实节点。
...
## 统一读取快照协议（timeline v3 / expand v2）
...
时间线响应顶层只包含事件包，不包含旧的扁平 `nodes` / `relations`：
...
- 相机输入分工：源内滚轮属于时间飞行（捕获阶段消费，非 passive），OrbitControls 保留拖拽
  旋转/平移与 pinch 缩放（damping 与 zoom-to-cursor 保留）。飞行 rig 每帧把 camera 与
  controls.target 沿轴一起平移深度增量，与 OrbitControls 正交组合，无手势分类器。飞行的
  窗口跟随经与按钮相同的 moveTimeline 意图管线翻页，永不阻塞相机。
- 事件包按计数轴投影摆位（z = −序数 × 每事件长度，横向按包身份确定性散布）；呈现的
  近大远小/明暗由场景按相机飞行深度每帧计算，穿过 mesh、DOM 卡片与关系三层。
- 一次窗口切换只有一个窗口 revision 和一个共享 `startedAt`。同页事件及其实体同时开始插值，
  时间顺序由目标深度与比例表达，不使用逐节点延迟，因此不会出现一个个弹出或中间空白帧。
- 每个活动窗切换按稳定节点/关系 ID 计算 `retained / entering / exiting`：保留对象的轴位
  是绝对的、通常不移动；飞行态下新对象在其轴位就地凝现（0→1 透明度与缩放，无出生点位移、
  无弧线），真正失去全部 owner 的对象原地溶解淡出，动画结束后才释放。非飞行过渡保留原
  中央长入/外缘退出编排。共享实体和卡片 DOM 只要仍被引用就复用原对象。
- 相同场景签名复用同一个数据对象；phase、统计、相同窗口 settle 和预取不触发可见拓扑变化。
  像素比只有在设备/负载档位真实变化时更新；卡片用 keyed reconcile，不整体删除重建。
- 过渡期间前进/后退按钮禁用（飞行跟随在 busy 时自动等待并重试）。新页完成协议校验和原子
  接纳前，当前图谱、游标和连线保持可见；失败时无需“恢复整图”，因为场景从未先行退场。
- 两个方向的窗口移动都从当前插值位置继续，不跳回起点；稳定态始终只有一个配置上限内的
  活动窗口。
...
- 事件与实体常驻卡片开关默认都开启，但实际卡片数量仍受视口预算和碰撞回填约束；事件无论
  ...
  稳定分布；详情态来源光核退场，星云转为带柔和辉光的近景环境层并保持可见。事件星使用渲染层
  3/4；每个实体都使用渲染层 2 的
...
- 星云和入场动画有明确结束时间；场景稳定后暂停 RAF 与 WebGL 渲染。详情推进通过 shader
  uniform 完成近景亮度变化，不反复上传整段透明度缓冲；大尺寸辉光按像素比设置硬上限。
- 画布只有一个键盘焦点；方向键按稳定空间顺序游走，Enter/Space 锁定，Escape 清除。
- 星云粒子另设硬上限：桌面 12000、移动 4000；这与事实节点预算相互独立。
...
| 源内滚轮 | 保持 | 解除场景锁定，mini 面板保持 | 沿计数轴飞行（惯性 + 轴端墙） | 窗口跟随/预取按低水位补页 | 窗口随相机前进/回退 |
| 总览滚轮 / pinch | 保持 | 保持 | 平滑缩放 | 否 | 否 |
| 拖动 | 保持 | 保持 | 旋转/平移并刹停飞行惯性 | 否 | 否 |
| 下一段时间按钮 | 保持 | 否 | 相机滑翔到新窗口，页就地凝现/溶解 | 缓存不足时至多一页 | 固定上限时间窗前进 |
| 上一段时间按钮 | 保持 | 否 | 相机滑翔到新窗口，页就地凝现/溶解 | 较近缓存已退休时至多一页 | 固定上限时间窗后退 |
| 已到时间线末尾后继续前进 | 保持 | 否 | 相机停在轴端墙 | 否 | 否 |
...
- 时间线 v3 无扁平旧字段；续页缺少 `snapshot_id` 必须返回 422。
- 每个事件包的 `ordinal` 是快照内 canonical 探索序（时间倒序 → 叙事 rank → id）中的位置：
  页内严格递增、小于 `total_events`、跨页与双向读取一致；同刻整源（书籍导入）按叙事 rank
  分页。缓存拒收与已缓存窗口序数重叠的游标相邻页。
- 同一快照的根页重试产生相同 `page_id` 和事件包；从任意包游标恢复时无重复、无遗漏。
- 来源版本变化返回 409，客户端清空旧 source 会话，不混合载荷。
- timeline/expand 必须共享同一快照；旧令牌、跨锚点游标、无进展游标和响应时间边界变化
  都必须在工作集修改前拒绝。
...
- hover、click 和拖动不触发 API；只有明确来源入口、双侧低水位预取、飞行窗口跟随/底部页
  按钮缓存未命中和详情“探索更多”可以查询。源内滚轮本身只改飞行状态，翻页始终经由与按钮
  相同的意图管线。
- 可见窗口设为 2 时网络请求仍可一次读取 6 个事件，但按钮按 `[0,1] -> [2,3] -> [4,5]` 连续推进，
  不遗漏中间事件；缓存容量设为 96 时首屏仍只填充水位，不以容量上限为目标连续拉满网络。
- 默认窗口一次切换 6 个事件；同页所有事件与实体共用一个动画起点，近大远小由相机相对
  presence 每帧表达，禁止逐节点 start delay。过渡期间按钮禁用，不会并发请求或跨越缓存顺序。
...
- 新页失败、迟到或快照不匹配时，当前窗口和安全游标都不改变；服务端返回末页且用户抵达末端后，
  下一页按钮禁用且不再请求，飞行停在轴端墙上（pinch 缩放不受影响）。
