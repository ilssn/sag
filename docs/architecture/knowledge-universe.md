...
4. 焦点关系片内的事件卡片完整呈现；hover 或锁定时，当前一跳端点随关系一起可读，不能出现亮线却没有端点。
...
flowchart LR
  ...
  OBJECTS --> FOCUS["相机深度与焦点关系片<br/>约 3–5 个连续事件"]
  FOCUS --> DOM["DOM 卡片层"]
```

网络分页、纯数据缓存、场景窗口和卡片阅读层是四个独立概念：

- 分页 20 不等于一次替换 20 个场景节点；
- 缓存 1000 不等于创建 1000 个 Three.js 对象；
- 窗口 50 表示最多 50 个事件及其事实闭包参与星体网络，不等于同时展开 50 张卡片；
- 关闭卡片不删除事件、实体和关系。
...
卡片数量不作为第二套用户配置。探索态由相机深度连续形成焦点关系片；累积态由新增批次和
当前关系焦点决定阅读层。设备预算只作为渲染安全边界，不参与知识优先级或用户配置语义。

## 5. 焦点关系片与关系焦点

DOM 阅读层直接由场景中已经存在的事实网络推导，不维护第二个“卡片候选列表”，也不运行
碰撞算法重新选择知识。

### 5.1 普通探索

1. 窗口内全部事件、实体和真实关系都作为 WebGL 星体网络存在。
2. 事件与相机的相对深度决定粒子、事件星、完整卡片和边缘淡出的连续生命周期。
3. 中央安全视区通常自然形成约 3–5 个连续事件的焦点关系片；它是视觉结果，不是固定配额。
4. 普通实体保持为星体和关系端点，避免实体标签与事件卡片同时铺满屏幕。
5. 事件卡片的元信息、标题、摘要和操作区始终作为一个整体缩放、渐显和渐隐，禁止分阶段插入。

### 5.2 hover 或锁定

1. 当前事件或实体的一跳事实网络接管视觉注意力。
2. 一跳实体在事件周围使用确定性的径向槽位展开紧凑卡片；布局、相机和时间位置不变化。
3. 当前关系与共享实体桥提升显著度，无关节点和关系降透明度。
4. hover 移开恢复普通探索；点击锁定同一焦点；再次点击、点击空白或继续滚动解除。
5. 普通点击不打开详情；只有卡片上的显式入口打开 mini 详情工作区。

### 5.3 卡片锚定

- 事件星位于走廊中心，卡片优先向远离走廊中心的一侧展开，不遮挡新出现的星体；
- 正常生命周期不把已离开自然投影区的卡片硬夹在屏幕边缘，而是继续淡出；
- hover/锁定卡片保持确定性槽位，反复进入不会跳边或重排；
- 搜索、问答或详情面板只改变安全取景区域，不改变事实坐标。
...
- 事件轨迹由稳定时间序、窄幅确定性走廊和相机深度决定；
- 新事件从镜头中心远处由粒子凝成事件星与完整卡片；
- 时间序只推进纵深，不随探索轮次扩大横向散布；稳定哈希只提供轻微错落；
- 实体使用事件本地的确定性径向槽位，共享实体保留真实跨事件桥；
- 近大远小按“节点深度 − 相机飞行深度”实时计算；
- 越过阅读区后在自然轨迹上向外围淡出，不吸附到视口边缘；
- 正向与回退使用同一确定性轨迹的逆过程；
- 相机只在输入期间运动，空闲时渲染器休眠。

### 6.4 累积投影

- 新批次优先靠近已有共享实体，只有新增节点参与至多 42 tick / 720ms 的局部收敛；
- 未退出窗口的旧节点固定在已有世界坐标，沉降完成后新增节点也冻结；
- 新批次进入后相机只框入新增节点和至多 4 个最近桥接邻居；
- 搜索/问答面板存在时，取景中心避开面板；
...
4. 探索态同一事件序号始终映射到同一世界坐标，不因分页、hover 或锁定变化。
5. 普通实体卡片不常驻；关系焦点卡片只来自当前节点的真实一跳端点。
6. 关系只在两端均存在时渲染。
7. hover/锁定不会改变窗口 revision、布局或相机。
8. 预取响应不会改变相机或活动窗口。
9. 完全重复线索批次不会移动既有节点。
10. 累积态新增批次沉降时，既有节点坐标保持不变。
11. 返回探索后窗口成员与飞行深度等于进入累积态前的快照。
12. 空闲场景 `data-universe-renderer="sleeping"`。
13. 策略切换后没有跨策略坐标记忆和 ghost 节点。

场景根节点通过 `data-universe-*` 暴露只读诊断，包括事件/实体/关系数量、窗口 revision、
飞行深度、实际卡片数量、焦点关系、布局稳定性、渲染器休眠状态和 WebGL 预算。
