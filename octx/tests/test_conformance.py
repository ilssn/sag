...
def test_package_digest_uses_rfc8785_golden_value() -> None:
    ...
    assert package_digest(manifest) == "sha256:22778fa1775ed1393311b52c32a3a4c80ffa155a0728fedbb2b19bf47ffbec42"
...
def test_core_accepts_forward_compatible_minor_versions(tmp_path: Path) -> None:
    package = _core_package(tmp_path)

    def mutate(manifest: dict) -> None:
        manifest["format_version"] = "1.1"
        manifest["x-optional-1-1-field"] = {"preserved": True}

    future_minor = repack(package, tmp_path / "core-1.1.octx", mutate_manifest=mutate)
    report = validate_octx(future_minor)

    assert report.core.valid
    assert report.valid
    assert report.core.version == "1.1"
...
def test_unknown_capability_keeps_core_readable_but_is_not_claimed_valid(tmp_path: Path) -> None:
    package = _core_package(tmp_path)
...
def test_issue_limit_must_be_positive(tmp_path: Path) -> None:
    with pytest.raises(ValueError, match="max_issues"):
        validate_octx(_core_package(tmp_path), max_issues=0)


def test_declared_missing_capability_file_does_not_invalidate_core(tmp_path: Path) -> None:
    package = _core_package(tmp_path)

    def mutate(manifest: dict) -> None:
        manifest["capabilities"] = {"chunks": {"version": "1.0"}}
...
def test_standard_payload_without_declaration_is_a_core_error(tmp_path: Path) -> None:
    package = _core_package(tmp_path)
