...
3. **SAG-structured v0.1 Capability**（`sag-structured-v0.1.md`）：完整的 chunks、events、entities、关系及可选向量。
4. **工具链概览**（`tooling-lifecycle.md`）：工具分层及 Asset、Release、Package、Installation 等核心对象。
5. **构建与集成**：`creating-octx.md`、`opening-validating-octx.md`、`importing-octx.md`、`exporting-octx.md` 分别说明创建、校验、导入和导出。
6. **Python API**（`api/overview.md`）：按功能拆分的创建、读取、校验、安全解包和 CLI 文档。
7. **机器可读 Schema**（`schemas/README.md`）：JSON Schema Draft 2020-12 与语义校验边界。
8. **领域词汇表**（`GLOSSARY.md`）：OCTX、SAG 和 zleap-sag 的统一术语。
