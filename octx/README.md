...
4. **Python API**（`python-api.md`）：安装、创建、读取、校验、安全解包和 CLI。
...
`source` 不会被修改。持续使用同一个 workspace，可以保留 Asset、Document 和 Release 的稳定身份。向量读取需要安装 `octx[vectors]`。完整接口见 `python-api.md`。
