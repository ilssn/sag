...
> 状态：v1 设计基线。本文定义 OCTX Core 的规范行为；[机器可读 JSON Schema](schemas/README.md)、规范样例和一致性测试应与本文保持一致。
...
OCTX Core 不负责搜索、排序、Agent 协议、数据库映射或索引运行时。
...
1. **Core**：所有实现都必须理解的 manifest、知识文档、身份、版本和完整性。
2. **Capability**：Package 实际携带的一种版本化标准数据能力。
3. **Profile**：面向特定用途、需要额外一致性约束的显式能力组合。

Capability 不能自动推导 Profile。未知 Capability 不阻止消费者读取有效 Core；消费者也不能声称满足自己无法验证的 Profile。
...
├── data/
│   ├── chunks.jsonl                # chunks/1.0
│   ├── events.jsonl                # events/1.0
│   └── entities.jsonl              # entities/1.0
├── relations/
│   ├── chunk-events.jsonl          # events/1.0
│   └── event-entities.jsonl        # entities/1.0
├── vectors/
│   ├── config.json                 # vectors/1.0
│   ├── chunks.arrow
│   ├── events.arrow
│   └── entities.arrow
└── extensions/
...
- 禁止加密条目。
- v1 只要求消费者支持 ZIP STORE 和 DEFLATE；ZIP64 必须可用于大型 Package。

解包器必须在写入前校验目标路径，并检测目标文件系统上的名称冲突，不能覆盖目录外文件或已有不相关文件。消费者必须设置单文件大小、总展开大小、条目数量和压缩比等本地资源上限。

## 5. Manifest

`manifest.json` 是 UTF-8 JSON object。最小 Core manifest：
...
| --- | --- |
| `format` | 固定为 `octx` |
| `format_version` | Core schema 版本，v1 为 `1.0` |
...
支持当前 major 的消费者必须允许未知可选字段、读取已知字段，并在无损重写 Package 时保留未知字段。未知字段不能改变已知字段语义或单独满足 Capability / Profile。
...
| Capability | 必需文件 | 依赖 |
| --- | --- | --- |
| `chunks/1.0` | `data/chunks.jsonl` | Core |
| `events/1.0` | `data/events.jsonl`, `relations/chunk-events.jsonl` | `chunks/1.0` |
| `entities/1.0` | `data/entities.jsonl`, `relations/event-entities.jsonl` | `events/1.0` |
| `vectors/1.0` | `vectors/config.json` 和至少一个标准 `.arrow` 文件 | 对应目标记录能力 |

relations 不是独立 Capability。标准路径出现时必须与对应 Capability 声明一致；生产者私有数据放入 `extensions/`，不能占用或重新解释标准路径。

每个 Package 中每类 v1 标准 JSONL 文件至多一份，不按文档拆分或分片。未来分片必须使用新的 Capability 版本。
...
- `format_version`：Core schema，`major.minor`。
...
同一 schema major 的新 minor 只能增加可选内容或澄清语义，必须向后兼容。需要新增必需行为、改变已有字段语义或删除既有内容时升级 major。

消费者行为：

- 支持 Core `1.x` 的消费者读取同 major 的已知内容并保留未知可选字段。
- 未知 Capability 不阻止 Core 读取。
- Profile 依赖未知内容时不得宣称 Profile 有效。
- 不支持的 Core major 必须明确拒绝，不能猜测解释。
...
Core 无效时不得安装为原 Asset。工具可以在用户明确选择后提取仍可读取的 Markdown，并使用新的 Asset 和文档身份创建派生资产。
