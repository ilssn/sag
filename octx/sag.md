...
SAG 是一套面向个人与 Agent 的开源知识库应用，也是一套同名的原创检索架构。Open Context 由 SAG 首先实现，但 OCTX 是独立标准，不要求使用 SAG。
...
| **`zleap-sag`** | 实现 SAG 检索架构的 Python 引擎，提供抽取、检索以及 SAG 专属的 OCTX 导入和导出适配。 | 依赖通用 `octx` 包，并增加 `import_octx()` 与 `export_octx()`。 |
...
- **SAG-structured Profile** 负责声明一个 OCTX Package 已携带完整、可直接导入 SAG 结构层的数据。

因此，SAG 是 OCTX 的首个生产者和消费者，而不是 OCTX 的唯一实现。其他知识系统可以直接采用 OCTX，并实现自己的导入、导出和检索适配。

## 相关入口

- [SAG 开源项目](https://github.com/Zleap-AI/SAG)
- [SAG 论文](https://arxiv.org/abs/2606.15971)
- [SAG-structured Profile](sag-structured-v0.1.md)
