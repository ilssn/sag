...
一个 zleap 实例只需**一个** FastMCP server：具体服务哪个信源由「作用域」
...
def build_source_mcp(*, stateless_http: bool = False) -> FastMCP:
    """构造 zleap 的信源 MCP server（工具从 contextvar 取当前信源作用域）。
    ...
    mcp = FastMCP(
        "zleap-source",
        instructions=(
            "zleap 单个信源的 MCP 端点：search 检索证据、get_entity 查实体上下文、"
...
async def serve_stdio(source_id: str) -> None:
    """stdio 入口：`ZLEAP_MCP_SOURCE_ID=<id> python -m zleap_api.mcp.server`。
...
def _main() -> None:
    ...
    source_id = os.environ.get("ZLEAP_MCP_SOURCE_ID", "").strip()
    if not source_id:
        raise SystemExit("请设置 ZLEAP_MCP_SOURCE_ID=<信源 id>")
