...
所有配置项均可通过环境变量 `ZLEAP_*` 或 `.env` 覆盖。设计上区分三类后端：

- **zleap 元数据库**（用户 / 信源 / 文档 / 会话）：`database_url`
...
class Settings(BaseSettings):
    ...
    # ── zleap 元数据库 ───────────────────────────────────────────────────
    database_url: str = "sqlite+aiosqlite:///./.data/zleap.db"
    ...
    sag_pg_user: str = "zleap"
    sag_pg_password: str = "zleap"
    sag_pg_database: str = "zleap"
