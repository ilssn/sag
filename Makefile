...
desktop-install: ## 安装桌面壳依赖（npm，Electron）
	cd apps/desktop && npm ci

desktop-dev: ## 桌面开发模式（先 make api 起 venv sidecar 或由壳按 SAG_SIDECAR_COMMAND 拉起）
	cd apps/desktop && npm run stage:frontend && npm run dev

desktop-sidecar: ## 冻结 FastAPI sidecar（PyInstaller onedir → binaries/sidecar）
	cd apps/desktop && python3 scripts/build_sidecar.py

desktop-build: ## 生产构建（web 静态导出 + sidecar 冻结 + electron-builder）
